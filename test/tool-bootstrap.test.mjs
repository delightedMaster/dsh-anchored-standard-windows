...
import { apply, name } from '../preset/anchored-standard-windows/tool-bootstrap.mjs'
...
test('an explicit empty compactionTools list is valid', async () => {
  const { listeners } = register({ ...config, compactionTools: [] })
  const tools = [{ name: 'bash' }, { name: 'str_replace_editor' }, { name: 'read' }]
  const events = [
    { type: 'assistant/message', seq: 1, data: {} },
    { type: 'compaction/end', seq: 2 },
